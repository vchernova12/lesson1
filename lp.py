...
def lp_upper_bound(new_billboard_contraint, in_stock, prob, dbb_vars, avg_display, constant_plus=1):
    bound = avg_display + constant_plus
    for bb in new_billboard_contraint:
        if in_stock[bb] > bound:
            prob += p.lpSum(new_billboard_contraint[bb]*dbb_vars[bb]) <= bound
        else:
            prob += p.lpSum(new_billboard_contraint[bb]*dbb_vars[bb]) <= in_stock[bb]
...
def lp_lower_bound(new_billboard_contraint, billboards_costs, in_stock, budget_for_lp, avg_display, prob, dbb_vars):
    lp_bb_costs = 0.0
    for bb in new_billboard_contraint:
    ...
    for bb in new_billboard_contraint:
        if in_stock[bb] > lp_avg_display:
            prob += p.lpSum(new_billboard_contraint[bb] *dbb_vars[bb]) >= lp_avg_display
            print(f'for{bb} lower bound set to {lp_avg_display}')
        else:
            prob += p.lpSum(new_billboard_contraint[bb] *dbb_vars[bb]) >= avg_display
            print(f'for{bb} lower bound set to {avg_display}')
    return prob


def lp_budget_contraint(billboards_costs, new_billboards, budget_for_lp, prob, dbb_vars):
    max_budget = budget_for_lp*1.01
    min_budget = budget_for_lp*0.97
    prob += p.lpSum([billboards_costs[i]*dbb_vars[i] for i in new_billboards]) >= min_budget
    prob += p.lpSum([billboards_costs[i]*dbb_vars[i] for i in new_billboards]) <= max_budget
    return prob


def lp_contraints_constractor(budget, billboards, in_stock, billboards_costs, prob, dbb_vars, new_billboards):
    budget = budget/100
    print(budget)
    ...
    upper_bound = lp_upper_bound(new_billboard_contraint, in_stock, prob, dbb_vars, avg_display)
    budget_for_lp = lp_budget(bb_with_minimal_stock, budget, billboards_costs, in_stock)
    lower_bound = lp_lower_bound(new_billboard_contraint, billboards_costs, in_stock, budget_for_lp, avg_display, prob, dbb_vars)
    budget_contraint = lp_budget_contraint(billboards_costs, new_billboards, budget_for_lp, prob, dbb_vars)
    return upper_bound, lower_bound, budget_contraint, bb_with_minimal_stock
...
def lp_constractor(budget, billboards, in_stock, billboards_costs):
    ...
    dbb_vars = p.LpVariable.dicts("Digital", new_billboards, cat=p.LpInteger )
    # 
    # Objective Function 
    

    prob += p.lpSum([billboards_costs[i]*dbb_vars[i] for i in new_billboards])
    #print(dbb_vars)
    #print(billboards_costs)
    #print([billboards_costs[i]*dbb_vars[i] for i in new_billboards])
    # Contraints: 
    upper_bound, lower_bound, budget_contraint, bb_with_minimal_stock = lp_contraints_constractor(budget, billboards, in_stock, billboards_costs, prob, dbb_vars, new_billboards)
    #print(prob)
    ...
    result_value = bb_with_minimal_stock
    for v in prob.variables():
        if v.varValue > 0:
            print(v.name, "=", v.varValue)
            result_value.update({f'{ v.name[8:]}': int(v.varValue*100)})
    #result_ots={}

    #for v in v in prob.variables():
    #result_value.update({f'{ v.name[8:]}': int(v.varValue*100*)})
